	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  document.getElementById('imgInput').addEventListener('change', async function (e) {
		 ...
		 // ⭐⭐ hidden input 생성해서 form에 추가
		    ...
		    insertedImages.push(fileName);
		    localStorage.setItem('insertedImages', JSON.stringify(insertedImages));
	...
	window.triggerImageUpload = function () {
	  ...
	  window.submitPost = function () {
		    ...
		    imgs.forEach(img => {
		      const src = img.src;
		      const fileName = src.substring(src.lastIndexOf("/") + 1);
		      const exists = form.querySelector(`input[name="imgid"][value="${fileName}"]`);
		      if (!exists) {
        ...
        <table border="0" width="80%">
            ...
            <tr>
                <td colspan="2" align="center">
                    <button type="submit" onclick="return submitPost();">수정 완료</button>
                </td>
            </tr>
        </table>
    </form>
